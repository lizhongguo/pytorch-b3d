...
class SpatioTemporalResLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, layer_size, block_type=SpatioTemporalResBlock, downsample=False):
        ...
        self.block1 = block_type(
            in_channels, out_channels, kernel_size, downsample)

        # prepare module list to hold all (layer_size - 1) blocks
        self.blocks = nn.ModuleList([])
        for i in range(layer_size - 1):
            # all these blocks are identical, and have downsample = False by default
            self.blocks += [block_type(out_channels,
...
class R2Plus1DClassifier(nn.Module):
    ...
    def __init__(self, num_classes = 7, layer_sizes = [2,2,2,2], block_type=SpatioTemporalResBlock):
