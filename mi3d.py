...
class MaxPool3dSamePaddingWithIdx(nn.MaxPool3d):
    ...
    def forward(self, x, index, in_channels):
        # compute 'same' padding
        (batch, channel, t, h, w) = x.size()
        # print t,h,w
        out_t = np.ceil(float(t) / float(self.stride[0]))
        ...
        x = F.pad(x, pad)
        x = torch.flatten(x, 2)
        y = torch.gather(x, 2, torch.flatten(index, 2)).view(index.size())
        return y
...
class MInceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        """
        assert in_channels == 5, 'input must be rgb+flow'

        if final_endpoint not in self.VALID_ENDPOINTS:
            raise ValueError('Unknown final endpoint %s' % final_endpoint)

        super(MInceptionI3d, self).__init__()
        ...
        self.end_points[end_point] = Unit3D(in_channels=3, output_channels=64, kernel_shape=[7, 7, 7],
        ...
        end_point = 'Logits'
        self.avg_pool = nn.MaxPool3d(kernel_size=[4, 7, 7],
                                     stride=(1, 1, 1))
    ...
    def forward(self, x):
        flow_x = x[:,3,:,:,:]
        flow_y = x[:,4,:,:,:]
        flow_x = 1e-1*F.avg_pool3d(flow_x,kernel_size=2,stride=2)
        flow_y = 1e-1*F.avg_pool3d(flow_y,kernel_size=2,stride=2)
        ...
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                ...
                if end_point == 'Conv3d_1a_7x7':
                    x_x = torch.einsum('ncthw,nthw->ncthw',x[:,:32,:,:,:],torch.exp(flow_x.abs()))
                    x_y = torch.einsum('ncthw,nthw->ncthw',x[:,32:,:,:,:],torch.exp(flow_y.abs()))
